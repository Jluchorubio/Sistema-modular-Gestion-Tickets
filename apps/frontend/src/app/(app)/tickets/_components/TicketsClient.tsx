...
function CreateModal({ moduleId, onClose }: { moduleId: string; onClose: () => void }) {
  ...
  /* ── Sync damage type into form ── */
  function selectDamageType(dt: DamageType | null) {
    setSelectedDamageType(dt);
    setForm((f) => ({
      ...f,
      damage_type_id:            dt?.id ?? undefined,
      custom_damage_description: undefined,
      // auto-suggest priority from damage type (overridable)
      priority: dt?.default_priority as TicketPriority ?? f.priority,
    }));
  }

  function handleDamageCategoryChange(catId: string) {
    setSelectedDamageCategory(catId);
    selectDamageType(null);
  }
  ...
  function clearAsset() {
    ...
    setForm((f) => ({ ...f, asset_id: undefined }));
  }
  ...
  const sect: React.CSSProperties = { borderTop: '1px solid #f1f5f9', paddingTop: 14, display: 'flex', flexDirection: 'column', gap: 12 };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,.45)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 560, maxHeight: '92vh', overflowY: 'auto', padding: '28px 28px 24px', position: 'relative' }} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* ── Damage type section ── */}
          <div style={sect}>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '.06em' }}>Tipo de daño / incidencia</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const dt = activeTypes.find((d) => d.id === e.target.value) ?? null;
                    selectDamageType(dt);
                  }}
            ...
            {/* Custom damage description when is_other */}
            {selectedDamageType?.is_other && (
              <div>
                <label style={lbl}>Descripción del daño *</label>
                <textarea
                  value={form.custom_damage_description ?? ''}
                  onChange={(e) => setForm((f) => ({ ...f, custom_damage_description: e.target.value }))}
            ...
            {/* Priority hint from damage type */}
            {selectedDamageType && !selectedDamageType.is_other && (
              <p style={{ margin: 0, fontSize: 10, color: '#64748b' }}>
                Prioridad sugerida por tipo de daño: <strong style={{ color: '#0e2235' }}>{selectedDamageType.default_priority}</strong> — puedes ajustarla abajo.
              </p>
            )}
          </div>
          ...
          {/* ── Asset linkage ── */}
          <div style={sect}>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '.06em' }}>Equipo / activo afectado <span style={{ fontWeight: 400, textTransform: 'none', fontSize: 10 }}>(opcional)</span></p>

            {selectedAsset ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, border: '1.5px solid #0e2235', background: '#f0f4f8' }}>
                <Monitor size={16} style={{ color: '#0e2235', flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ margin: 0, fontSize: 10, color: '#64748b' }}>
                    ...
                    {selectedAsset.category_name ?? ''}{selectedAsset.environment_name ? ` · ${selectedAsset.environment_name}` : ''}
                  </p>
                </div>
                <button type="button" onClick={clearAsset} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', padding: 2 }}><X size={13} /></button>
              </div>
            ) : (
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p style={{ margin: 0, fontSize: 10, color: '#94a3b8' }}>
                            {a.serial_number ? `S/N: ${a.serial_number}` : a.qr_code}
                            {a.environment_name ? ` · ${a.environment_name}` : ''}
                          </p>
                        </div>
          ...
          <div style={sect}>
            <p style={{ margin: 0, fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '.06em' }}>Clasificación</p>
