...
export class SystemModulesService {
  ...
  async findAll(userId: string) {
    ...
    if (profile?.is_superadmin) {
      return this.db.query<any[]>(
        `SELECT id, name, slug, description, type, image_url, color, is_active,
                ...
                access_mode, assignment_mode, priority_editors, created_at,
                true AS has_access
    ...
    return this.db.query<any[]>(
      `SELECT DISTINCT m.id, m.name, m.slug, m.description, m.type, m.image_url,
              ...
              m.access_mode, m.assignment_mode, m.priority_editors, m.created_at,
              (m.access_mode = 'open'
  ...
  async findOne(id: string) {
    const rows = await this.db.query<any[]>(
      `SELECT m.id, m.name, m.slug, m.description, m.type, m.image_url, m.color,
              ...
              m.access_mode, m.assignment_mode, m.priority_editors,
              m.auto_close_hours, m.waiting_timeout_hours, m.approval_timeout_hours, m.max_reopen_count,
  ...
  async updateModule(id: string, dto: Record<string, unknown>) {
    const {
      ...
      access_mode, assignment_mode, priority_editors,
      auto_close_hours, waiting_timeout_hours, approval_timeout_hours, max_reopen_count,
    } = dto as any;
    const fields: string[] = [];
    const values: any[] = [];
    let idx = 1;
